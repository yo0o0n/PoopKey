...
typedef struct ToiletStall {
    ...
    int last_ir_time;
    int last_flush_time;
    bool is_checked_broken;
    int last_tissue_time;
...
void sendESP(int target, (char *)msg);
...
void checkMagnetic(TS stall)
{
    bool current = sensing(stall.magnetic_door);
    if (current == true) {        
        ...
        if (stall.is_occupied == true) {
            ...
            if (stall.last_open_time != -1) {
                if ( (HAL_GetTick() - stall.last_open_time) >= CHANGE_OCCUPIED_TERM) { // GetTick()/(1000*60)%60 ms -> min
